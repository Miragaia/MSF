...
alfa = 5*math.pi/180       #---> (aleterar o 5 para outro valor em graus)
v0 = 1                   #---> alterar (aprsentar em m/s)
v0x = v0*math.cos(alfa)  #---> alterar (aprsentar em m/s)
v0y = v0*math.sin(alfa)  #---> alterar (aprsentar em m/s)

g = 9.80 
P = 0.4*745.7      #---> alterar(0.4 é em cv se for em W apresentar apenas o valor)
...
## Alínea b)
for i in range(N):
    if x[i+1] > 2000-2*dt and  x[i+1] < 2000+2*dt:       #tempo a percorrer 2Km    ---> alterar os 2km(2000) (apresentar em metros)
