...
def main():
    ...
    vt = 60 # velocidade terminal
    D = g/vt**2 #calculo do D para a resistência do ar
    t[0] = t0  # instante inicial
    vx[0] = v0 * 0  # velocidade inicial eixo x
    vy[0] = v0 * 1  # velocidade inicial eixo y
    x[0] = 0  # posição inicial eixo x
    y[0] = 800  # posição inicial eixo y
    ...
    for i in range(n):
        ...
        vy[i+1] = vy[i] + ay[i]*dt # alterar a velocidade eixo y
        vx[i+1] = vx[i] + ax[i]*dt # alterar a velocidade eixo x
