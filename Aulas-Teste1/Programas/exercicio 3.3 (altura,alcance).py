...
def main():
    ...
    alpha = np.deg2rad(10)  # angulo com a horizontal ######mudar
    v0 = 100 / 3.6  # velocidade inicial              ######mudar
    vt = 100*1000/3600 # velocidade terminal          ######mudar
    D = g/vt**2 #calculo do D para a resistência do ar #######mudar
    t[0] = t0  # instante inicial
    vx[0] = v0 * np.cos(alpha)  # velocidade inicial eixo x  
    vy[0] = v0 * np.sin(alpha)  # velocidade inicial eixo y
    x[0] = 0  # posição inicial eixo x                ######mudar
    y[0] = 0  # posição inicial eixo y                ######mudar
